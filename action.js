...
Toolkit.run(async tools => {
  ...
  try {
    execSync('curl https://raw.githubusercontent.com/back4app/parse-cli/back4app/installer.sh | sudo /bin/bash').toString().trim()
    execSync('mkdir -p ~/.back4app').toString().trim()
    execSync(`echo -e "machine parsecli.back4app.com\n  login default\n  password ${b4aApiKey}" > ~/.back4app/netrc`).toString().trim()

    if(appName && appName.length > 0) {
      execSync(`b4a default "${appName}"`).toString().trim()
    }
